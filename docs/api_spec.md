...
| 모듈 키 | "ready" 조건 |
|---------|------------|
| `data` | `returns.parquet`, `raw_features.parquet`, `features.parquet` 모두 읽을 수 있음 |
...
- PPO 모델(`models/ppo_*_risk.zip`)을 로드해 실제 비중 반환
...
| `reasoning_context` | `list[ReasoningEvent]` | target_date ±3일 범위 이벤트 reasoning trace (top-level) |
| `message` | `str` | 상태 설명 메시지 |
...
| `market_regime_comparison` | regime × strategy Two-way ANOVA — Factor 1: rate_hike·recovery·bull (균형 3×3), 국면·전략 주효과 + 교호작용 | 강유영 (`anova.py`) |
...
| 엔드포인트 | 담당 | 목표 | 연결 모듈 |
|-----------|------|------|---------|
| `POST /optimize` | 박지민 | PPO 비중 실제 반환 | `models/ppo_*_risk.zip` |
...
**Sprint 3 수정 계획**:
1. `schemas.py`: `AnovaResult.post_hoc: str` → `post_hoc: list[TukeyRow]` 변경 (박지민) — **미완료**
2. `schemas.py`: `TukeyRow` 신규 스키마 추가 (박지민) — **미완료**
3. `schemas.py`: `AnovaResult`에 `interaction: dict | None` + `strategy_effect: dict | None` 추가 (박지민) — **미완료**
4. `dashboard/app.py` `anova_page()`: `list[AnovaResult]` 처리로 수정 (강유영) — **완료 (Sprint 3)**
   - `strategy_comparison` 항목을 대표 지표(F, p, η²) 표시에 사용
   - `market_regime_comparison`의 `interaction` 교호작용 표시 추가
