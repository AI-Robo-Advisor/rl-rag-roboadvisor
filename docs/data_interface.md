...
| 결측치 | 0개 |
| 실제 shape | `(1423, 10)` |
| 날짜 범위 | 2020-01-03 ~ 2025-12-30 (US–KR inner join 기준) |
...
`src/rl/env.py`에서 자체 정규화(Z-score 또는 rolling)를 수행해야 한다.
...
| 결측치 | 0개 |
| 실제 shape | `(1390, 40)` |
| 날짜 범위 | 2020-02-25 ~ 2025-12-30 (MACD 초기 NaN 제거 후) |
...
Look-ahead bias는 `src/rl/env.py`의 Walk-Forward 분리에서 처리한다.
